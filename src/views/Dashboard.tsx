...
export default function Dashboard() {
  ...
  const handleUpdate = async () => {
    if (!editTemplate || !formName.trim() || submitting) return;
    setSubmitting(true);
    try {
      setError(null);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          <span className="hidden sm:inline text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">{serverIp}</span>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <button
            onClick={() => { setShowCreate(true); setEditTemplate(null); setFormName(''); setFormMediaId(''); }}
        ...
        {error && (
          <div className="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm flex items-center justify-between">
            <span>{error}</span>
            <button onClick={() => setError(null)} className="text-red-400 hover:text-red-600 cursor-pointer ml-2">x</button>
          </div>
        )}
        ...
        {loading ? (
          // Animated spinner while fetching templates
          ...
          <div className="grid gap-3 sm:gap-4">
            {templates.map((tpl) => (
              <div key={tpl.id} className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4 hover:shadow-md transition sm:flex sm:items-center sm:justify-between">
