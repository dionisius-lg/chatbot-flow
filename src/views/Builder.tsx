      ...
      <header className="bg-white border-b border-gray-200 px-2 sm:px-4 py-2 flex items-center justify-between shrink-0 gap-1 sm:gap-3">
        ...
        <div className="flex items-center gap-1 sm:gap-3 min-w-0">
          ...
          <h1 className="text-xs sm:text-sm font-semibold text-gray-700 flex items-center gap-1 sm:gap-2 truncate">
            {loading ? (
              <>
                <svg className="animate-spin h-3 w-3 sm:h-4 sm:w-4 text-indigo-500 shrink-0" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-1 sm:gap-3 shrink-0">
          {templateId && (
            <button
              ...
              title="Refresh"
            >
              <svg className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${loading ? 'animate-spin' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <span className="hidden sm:inline text-[10px] text-gray-500 bg-gray-100 px-2 py-1 rounded">{serverIp}</span>
