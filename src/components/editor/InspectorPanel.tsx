  ...
  if (!selectedFlow || !flow) {
    return (
      <div className={containerClasses}>
        ...
        <div className="p-4 text-center text-gray-400 text-sm mt-20">
          Select a flow node to edit properties
        </div>
      </div>
    );
  }
      ...
      <div className="p-4 border-b border-gray-200">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1">
            {dirty && (
              ...
              >
                {savingFlow ? <><svg className="animate-spin h-3 w-3" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Saving...</> : 'Save'}
              </button>
            )}
            <button
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input type="text" value={`#${flow.id}`} disabled className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-400" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                type="number"
                value={editTimeout}
                onChange={(e) => { setEditTimeout(Number(e.target.value)); setDirty(true); }}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                min={0}
              />
            </div>
            <div>
              ...
              >
                ...
                {useFlowStore.getState().flows.filter(f => f.id !== flow.id).map((f) => (
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
              <input
                ...
                onChange={(e) => { setEditInitial(e.target.checked ? 1 : 0); setDirty(true); }}
            ...
            <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
              <input
                ...
                onChange={(e) => { setEditActive(e.target.checked ? 1 : 0); setDirty(true); }}
                className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
              />
              Active
            </label>
        ...
        {showNewDialog && (
          <div className="mb-3 p-3 bg-gray-50 rounded-lg border border-gray-200 space-y-2">
            ...
            <button
              ...
              disabled={(newDialogType !== 6 && !newDialogBody.trim()) || (newDialogType === 6 && !newMediaFile) || addingDialog}
            >
              {addingDialog ? <><svg className="animate-spin h-3 w-3" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Adding...</> : 'Add'}
            </button>
          </div>
        )}
