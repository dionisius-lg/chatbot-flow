...
export default function DialogEditor({ dialog, setError }: Props) {
    ...
    const handleSetHeaderText = async () => {
        if (!newHeaderText.trim() || savingAction) {
            return;
        }
        setSavingAction('header');
        setError?.(null);
    ...
    const handleDeleteHeader = async () => {
        ...
        if (!window.confirm('Delete header?')) {
            return;
        }
        setSavingAction('header');
        setError?.(null);
                ...
                <div className='px-3 pb-3 space-y-2 border-t border-gray-200 pt-2'>
                    <div className='grid grid-cols-2 gap-2'>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value={editSequence}
                                onChange={(e) => {
                                    setEditSequence(Number(e.target.value));
                                    setDirty(true);
                                }}
                                className='w-full border border-gray-300 rounded px-2 py-1 text-xs focus:ring-1 focus:ring-indigo-500 outline-none'
                                min={1}
                        ...
                        <>
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        setEditOptionTitle(e.target.value);
                                        setDirty(true);
                                    }}
                                    className='w-full border border-gray-300 rounded px-2 py-1 text-xs focus:ring-1 focus:ring-indigo-500 outline-none'
                    ...
                    {editTypeId === 5 && (
                        <div>
                            ...
                            <input
                                ...
                                value={editShortcode}
                                onChange={(e) => {
                                    setEditShortcode(e.target.value);
                                    setDirty(true);
                                }}
                        ...
                        <div>
                            ...
                            {editMediaFile && (
                                <div className='text-[10px] text-indigo-600 mt-1'>
                                    File selected: {editMediaFile.name}
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        >
                            ...
                            {flows
                                .filter((f) => f.id !== dialog.bot_flow_id)
                                .map((f) => (
