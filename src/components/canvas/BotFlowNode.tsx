    ...
    >
      ...
      <div className="px-3 py-2 border-b border-gray-100 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {isInitial && <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-medium">START</span>}
        </div>
      </div>

      {/* ─── Flow Type Badge ──────────────────────────────────────────────── */}
      <div className="px-3 py-2 flex items-center gap-2">
        <span className="text-xs px-2 py-0.5 rounded-full font-medium text-white" style={{ background: flowTypeColor }}>
          {flowTypeName}
        </span>
        {flow.timeout_duration > 0 && (
          <span className="text-xs text-gray-500">{'\u23F1'} {flow.timeout_duration}s</span>
        )}
      </div>
      ...
      {dialogs.length > 0 && (
        <div className="px-3 pb-2 space-y-1">
          {dialogs.slice(0, 4).map((dialog: BotDialog) => (
            <div key={dialog.id} className="flex items-center gap-1.5 text-xs text-gray-600 bg-gray-50 rounded px-2 py-1">
              <span>{dialogIcons[dialog.bot_dialog_type_id] || '\u{1F4AC}'}</span>
              <span className="truncate flex-1">
                {dialog.body 
                  ? (dialog.body.length > 40 ? dialog.body.substring(0, 40) + '...' : dialog.body)
                  : dialog.media 
                    ? (() => {
                        try {
                          const mediaObj = typeof dialog.media === 'string' ? JSON.parse(dialog.media) : dialog.media;
                          return mediaObj?.link 
                            ? (mediaObj.link.length > 40 ? mediaObj.link.substring(0, 40) + '...' : mediaObj.link)
                            : '(empty)';
                        } catch {
                          return '(empty)';
                        }
