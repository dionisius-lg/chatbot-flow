  ...
  const onConnectHandler = useCallback(
    (params: Connection) => {
      const newEdge = addEdge(
        { ...params, animated: true, style: { stroke: '#6366f1', strokeWidth: 2 } } as Edge,
        edges
      );
      setEdges(newEdge);
      onConnect(params);
        ...
        <div className="absolute top-12 sm:top-16 right-2 sm:right-4 z-20 bg-white rounded-xl shadow-xl border border-gray-200 p-3 sm:p-4 w-64 sm:w-72">
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              >
                {creatingFlow ? <><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>Creating...</> : 'Create'}
              </button>
            </div>
          </div>
        </div>
